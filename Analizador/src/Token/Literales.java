...
public class Literales extends JFrame implements ActionListener{
	...
	public void construirPnlInferior(){
		...
		String[] datos = {"TIPO","PALABRA","LINEA"};
		for(int i=0;i<3;i++){
			dtmTabla.addColumn("");
		}
	...
	public void construirVentana(){
		jfrVentana= new JFrame("LITERALES EN EL TEXTO");
	...
	private void ImpresionPalabras(){
		String[] datoFin=new String[3];
		for(int i=0;i<analizadorLex.cadena.size();i++){
			String[] cadena=(analizadorLex.cadena.get(i)).split(";");
			StringTokenizer st = new StringTokenizer(cadena[0]);
				if(st.nextToken().equals("Literales")){
					datoFin[0]=st.nextToken();
					datoFin[1]=cadena[1];
					datoFin[2]=cadena[2];
